...
export const diaryOwnerMatchCheck = async (diary_id) => {
  const connection = await pool.getConnection(async (conn) => conn);
  const diaryOwnerMatchCheckResult = await selectUserbyDiaryId(
    connection,
    diary_id
  );
  connection.release();
...
export const retrieveDiaryList = async (user_id) => {
  ...
  const connection = await pool.getConnection(async (conn) => conn);
  const retrieveDairyListResult = await selectDiaryListById(
    connection,
    user_id
  );
  connection.release();
  ...
  if (retrieveDairyListResult[0][0]) {
      ...
      for (let i = 0; i < retrieveDairyListResult[0].length; i++) {
          const updatedTimeUTC = dayjs(
              retrieveDairyListResult[0][i].updated_at
          ).utc();
...
export const retrieveDiaryDetail = async (user_id, diary_id) => {
  ...
  const isLikedInfo = await selectIsLiked(connection, [user_id, diary_id]);
  ...
  const retrieveDiaryContentResult = await selectDiaryContent(
    connection,
    diary_id
  );
  const retrieveDiaryHashtagResult = await selectDiaryHashtag(
    connection,
    diary_id
  );
...
export const retrieveDairyListPublic = async (user_id, type) => {
  switch (type) {
    case 0:
      // user_id 존재 체크
      const diaryOwner = await userIdCheck(user_id);
      ...
      const connection = await pool.getConnection(async (conn) => conn);
      const retrieveDairyListHomeResult = await selectDiaryListPublic(
        connection,
        user_id
      );
      ...
      for (let i = 0; i < retrieveDairyListHomeResult[0].length; i++) {
        const updatedTimeUTC = dayjs(
          retrieveDairyListHomeResult[0][i].updated_at
        ).utc();
        const updatedTimeKorea = updatedTimeUTC.tz('Asia/Seoul');
        retrieveDairyListHomeResult[0][i].updated_at =
          updatedTimeKorea.format('YYYY.MM.DD');
      ...
      connection.release();
      if (retrieveDairyListHomeResult[0][0]) {
        return response(baseResponse.SUCCESS, retrieveDairyListHomeResult[0]);
      } else {
        return errResponse(baseResponse.DAIRY_DIARYID_NOT_EXIST);
      }
    ...
    case 1:
      ...
      const retrieveDairyListPublicResult = await selectDiaryListPublic(
        connection1,
        user_id
      );
      ...
      //일단은 임시 방법. dayjs를 사용
      //시간을 한국 시간대로 변경해 YYYY-MM-DD HH:mm:ss 형식으로 변환하는 과정. 다른 방식으로도 변환 가능.
      for (let i = 0; i < retrieveDairyListPublicResult[0].length; i++) {
        const updatedTimeUTC = dayjs(
          retrieveDairyListPublicResult[0][i].updated_at
        ).utc();
        const updatedTimeKorea = updatedTimeUTC.tz('Asia/Seoul');
        retrieveDairyListPublicResult[0][i].updated_at =
          updatedTimeKorea.format('YYYY.MM.DD');
      ...
      connection.release();
      if (retrieveDairyListPublicResult[0][0]) {
        return retrieveDairyListPublicResult[0];
      } else {
        return errResponse(baseResponse.DAIRY_DIARYID_NOT_EXIST);
      }
  }
